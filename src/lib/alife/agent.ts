...
import { buildSubstrateNetwork } from './hyperneat';
...
export class Agent {
  ...
  // poc2.6: Hebbian plasticity — network changes during life
  readonly _plastic: boolean;
  // Lifetime apple count (for diagnostics)
  applesEaten = 0;
  ...
  constructor(
    ...
    // Under HyperNEAT, genome is a CPPN and the executable network is a
    // substrate DERIVED from it; otherwise it's the genome's direct wiring.
    // Hebbian config is always passed; NeuralNetwork no-ops when disabled.
    if (config.hyperneat.enabled) {
      this.network = buildSubstrateNetwork(
        genome, config.sensors, config.network, config.hyperneat, config.hebbian,
      );
    } else {
      this.network = new NeuralNetwork(genome, config.network, config.hebbian);
    }

    // Under plasticity the network is NOT frozen for life, so cached
    // measurements (descriptor, steer_score) cannot be memoised.
    this._plastic = config.hebbian.enabled;
  ...
  eat(): number {
    ...
    // Reward-modulated Hebbian plasticity (no-op unless hebbian.enabled).
    // Called EVERY tick (including empty ones): traces must advance each tick,
    // and a tick with no capture carries a small negative contrast against
    // the reward baseline. Driven from here so the learning signal stays
    // where the reward is known — and so no measurement path can trigger it.
    this.network.applyHebbian(bitten.length);
